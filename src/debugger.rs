use tondi_consensus_core::tx::{Transaction, TransactionInput, TransactionOutput};
use tondi_scriptexec::{TondiScriptExecutor, ExecCtx, Options, Stack};

pub use tondi_script::{define_pushable, script};
define_pushable!();
pub use tondi_txscript::Script as Script;
...
pub fn debug_script(script: tondi_txscript::Script) -> (TondiScriptExecutor, String) {
    let exec = TondiScriptExecutor::new(
        ExecCtx::Tapscript,
        Options::default(),
    );

    let last_opcode = "Script execution not yet implemented".to_string();
